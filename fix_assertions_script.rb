...
Find.find('.') do |path|
  if path =~ /.*\.rb$/
    ...
    text.each_line do |line|
      ...
      replace = build_simple_assert_difference_replace_regex(count_needed: true, parenthesis_needed: false)
      ...
      replace = build_simple_assert_difference_replace_regex(count_needed: false, parenthesis_needed: false)
      text = text.gsub(Regexp.new(find), replace)
